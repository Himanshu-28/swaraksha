...
import { Home, FileText, Calendar, User, LogOut, Stethoscope } from 'lucide-react';
import { signOut } from 'aws-amplify/auth';
import { useUser } from '../contexts/UserContext';

const NAV_ITEMS = [
  { icon: Home,     label: 'Home',           path: '/patient-home' },
  { icon: FileText, label: 'Transcriptions', path: '/patient-transcriptions' },
...
export default function PatientAppLayout() {
  ...
  return (
    <div className="flex min-h-screen bg-[#f8fafc]">
      ...
      {/* Mobile header avatar (top-right) — visible on mobile only */}
      <div className="md:hidden fixed top-4 right-4 z-40">
        <div className="border border-[#e2e8f0] flex items-center justify-center p-px rounded-full size-[36px] bg-[#dbeafe] cursor-pointer">
          <span className="font-bold text-[#196ee6] text-xs">{initials}</span>
        </div>
      </div>
    </div>
  );
